...
import type { SelectionRestore } from '../../../src/core/selection-restore'
...
export function useHighlightNavigation(
  ...
  /**
   * 从 SDK 获取高亮列表
   * 直接读取 SDK 内部的 activeRanges
   */
  const getHighlightsFromSDK = (): NavigableHighlight[] => {
    const sdkInstance = getSDKInstance()
    if (!sdkInstance) {
      return []
    }

    const selectionIds = sdkInstance.getAllActiveSelectionIds()
   ...
   */
  const refresh = (): void => {
    highlightList.value = getHighlightsFromSDK()
