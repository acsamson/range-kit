...
import type { SelectionTypeConfig } from '../../../src/core/selection-restore'
...
export function useSearchHighlight(options: UseSearchHighlightOptions): UseSearchHighlightReturn {
  ...
  const ensureTypeRegistered = (type: string): void => {
    const sdkInstance = options.getSDKInstance()
  ...
  const searchAndHighlight = async (
    ...
    for (const keyword of keywordArray) {
      ...
      try {
        const result = await sdkInstance.highlightTextInContainers(keyword, type, containers, {
  ...
  const clearSearchHighlights = (keywords?: string | string[]) => {
    ...
    if (keywords) {
      const keywordArray = Array.isArray(keywords) ? keywords : [keywords]
      for (const keyword of keywordArray) {
        sdkInstance.clearTextHighlights(keyword, containers)
        // 移除关键词
      ...
      sdkInstance.clearTextHighlights(undefined, containers)
