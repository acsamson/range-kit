...
export class SelectionEventHandlers {
  ...
  setupSelectionCompleteListener(): void {
    this.selectionChangeListener = () => {
      ...
      if (this.config.options.onSelectionComplete) {
        // 创建临时的选区数据（部分数据，用于事件通知）
        const tempData = {
          id: `temp-${Date.now()}`,
          text: range.toString(),
          type: this.config.options.defaultSelectionType,
          restore: {
            anchors: { startId: null, endId: null, startOffset: 0, endOffset: 0 },
            paths: { startPath: '', endPath: '', startOffset: 0, endOffset: 0, startTextOffset: 0, endTextOffset: 0 },
            multipleAnchors: { startAnchors: null, endAnchors: null, commonParent: null, siblingInfo: null },
            fingerprint: { tagName: '', className: '', attributes: {}, textLength: 0, childCount: 0, depth: 0, parentChain: [], siblingPattern: { position: 0, total: 0, beforeTags: [], afterTags: [] } },
            context: { precedingText: '', followingText: '', parentText: '', textPosition: { start: 0, end: 0, totalLength: 0 } },
          },
        } as unknown as SerializedSelection;

        const completeEvent: SelectionCompleteEvent = {
          range: range.cloneRange(),
          selection: tempData,
          defaultType: this.config.options.defaultSelectionType || DEFAULT_SELECTION_TYPE,
        };

        const tempInstance = this.config.createTempInstance(tempData);
        this.config.options.onSelectionComplete(completeEvent, tempInstance);
      }
    };
