...
export function useHighlightNavigation(options: UseHighlightNavigationOptions) {
  ...
  const refresh = useCallback(() => {
    ...
    const selectionIds = sdk.getAllActiveSelectionIds();
    const highlights: NavigableHighlight[] = [];

    for (const id of selectionIds) {
      const range = sdk.getActiveRange(id);
      if (range) {
        highlights.push({
          id,
          text: range.toString(),
  ...
  }, [getSDKInstance, currentIndex]);

  const goToIndex = useCallback((index: number) => {
    // Need latest list, so we call refresh first? 
    // Or rely on state. State might be stale if SDK updated.
    // Better to fetch from SDK again or rely on caller to refresh.
    ...
    if (autoScroll && highlight.range) {
       highlight.range.startContainer.parentElement?.scrollIntoView({
           behavior: 'smooth',
           block: 'center',
           inline: 'nearest'
       });
    }
  }, [getSDKInstance, loop, autoScroll, currentIndex, applyCurrentHighlightStyle, onHighlightStyleChange, onNavigate]);
