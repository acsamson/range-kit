...
该包提供了一组 Hooks 来管理选区、高亮和用户交互。

## useSelectionRestore

用于管理持久化选区的核心 Hook。它处理初始化、状态管理以及与核心 `SelectionManager` 的交互。
...
| 值 | 类型 | 描述 |
| :--- | :--- | :--- |
| `currentSelections` | `SerializedSelection[]` | 当前活动的持久化选区数组。 |
...
| `searchResults` | `SearchResultItem[]` | 当前搜索结果及匹配计数的数组。 |

---

## usePopover

用于相对于选区定位浮动 UI 元素的辅助 Hook。由 `floating-ui` 驱动。
...
| 值 | 类型 | 描述 |
| :--- | :--- | :--- |
| `show` | `(data) => void` | 在特定位置显示 Popover（通常来自 `onSelectionAction`）。 |
| `hide` | `() => void` | 隐藏 Popover。 |
| `visible` | `boolean` | 当前可见性状态。 |
| `floatingStyles` | `CSSProperties` | 应用于 Popover 元素的样式。 |
