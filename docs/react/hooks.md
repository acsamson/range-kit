...
This package provides a set of hooks to manage selections, highlights, and user interactions.

## useSelectionRestore

The core hook for managing persistent selections. It handles initialization, state management, and interaction with the core `SelectionManager`.

```typescript
import { useSelectionRestore } from '@range-kit/react';
...
| :--- | :--- | :--- |
| `currentSelections` | `SerializedSelection[]` | Array of currently active persistent selections. |
...
```typescript
import { useSearchHighlight } from '@range-kit/react';
...
| `searchResults` | `SearchResultItem[]` | Array of current search results and match counts. |

---

## usePopover

Helper for positioning floating UI elements relative to selections. Powered by `floating-ui`.

```typescript
import { usePopover } from '@range-kit/react';
...
| Value | Type | Description |
| :--- | :--- | :--- |
| `show` | `(data) => void` | Shows the popover at a specific position (usually from `onSelectionAction`). |
| `hide` | `() => void` | Hides the popover. |
| `visible` | `boolean` | Current visibility state. |
| `floatingStyles` | `CSSProperties` | Styles to apply to the popover element. |
