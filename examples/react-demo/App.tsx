...
function App() {
  ...
  const handleSearch = async () => {
    ...
    await searchAndHighlight(searchText);
  };
...
`;
  ...
  return (
    <div className="react-demo" style={{ maxWidth: '800px', margin: '0 auto', padding: '20px', fontFamily: 'sans-serif' }}>
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', borderBottom: '1px solid #ccc' }}>
        <h1>React Range Kit Demo</h1>
        <a href="/">Back to Home</a>
      </header>

      <div className="controls" style={{ background: '#f0f0f0', padding: '15px', borderRadius: '8px', marginBottom: '20px', display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
        <div className="control-group" style={{ flex: 1, minWidth: '200px' }}>
          <h3>Selection</h3>
          <button onClick={() => saveCurrentSelection()} style={{ padding: '5px 10px', cursor: 'pointer' }}>Save Selection</button>
          <div>Saved Selections: {currentSelections.length}</div>
        </div>
        ...
        <div className="control-group" style={{ flex: 1, minWidth: '200px' }}>
          ...
          />
          <button onClick={handleSearch} style={{ padding: '5px 10px', cursor: 'pointer', marginLeft: '5px' }}>Search</button>
          <button onClick={() => clearSearchHighlights()} style={{ padding: '5px 10px', cursor: 'pointer', marginLeft: '5px' }}>Clear</button>
        </div>

        <div className="control-group" style={{ flex: 1, minWidth: '200px' }}>
          <h3>Navigation</h3>
          <button onClick={goToPrev} disabled={total === 0} style={{ padding: '5px 10px', cursor: 'pointer', marginRight: '5px' }}>Prev</button>
          <span>{currentIndex + 1} / {total}</span>
          <button onClick={goToNext} disabled={total === 0} style={{ padding: '5px 10px', cursor: 'pointer', marginLeft: '5px' }}>Next</button>
        </div>
      </div>
      ...
      <div 
        ...
        ref={contentRef} 
        style={{ lineHeight: 1.6, padding: '20px', border: '1px solid #ddd', borderRadius: '4px', minHeight: '300px' }}
        dangerouslySetInnerHTML={{ __html: chuShiBiao }}
      />
    </div>
  );
}
