...
} from '../../../../../vue/hooks/use-selection-restore/types'
 ...
 *
 * @description
 * 提供选区行为的回调函数，用于响应用户的选区操作：
 * - onSelectionAction: 统一处理所有选区动作（创建、清除、点击、悬停等）
 * - onSearchHighlightInteraction: 处理搜索高亮的交互
 *
 * @example
 * ```ts
 ...
 * })
 *
 * // 在 useSelectionRestore 配置中使用
 * useSelectionRestore({
 *   onSelectionAction
 * })
 *
 * // 在 useSearchHighlight 配置中使用
 * useSearchHighlight({
 *   onSearchHighlightInteraction
 * })
 * ```
