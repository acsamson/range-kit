...
describe('vue utils/scroll', () => {
  ...
  it('sortHighlightIdsByDOMOrder sorts by DOM order using compareBoundaryPoints', () => {
    ...
    const r1 = createRangeForNode(t1, 0, 3)
    const r2 = createRangeForNode(t1, 4, 7)
    const r3 = createRangeForNode(t2, 0, 3)
  ...
  it('findScrollableAncestor finds the closest scrollable parent (vertical)', () => {
  ...
  it('scrollToRange uses container scroll when out of view', () => {
    ...
    scrollToRange(range)
    expect((container.scrollTo as unknown as ReturnType<typeof vi.fn>)).toHaveBeenCalled()
  })
  ...
  it('scrollToRange does not scroll container when already in view', () => {
    ...
    scrollToRange(range)
    expect((container.scrollTo as unknown as ReturnType<typeof vi.fn>)).not.toHaveBeenCalled()
  })

  it('scrollToRange uses window scroll when out of viewport and catches errors', () => {
    const el = document.createElement('span')
